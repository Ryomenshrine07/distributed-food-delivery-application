// Example/widget tests for the custom rider marker (Item 1, Customer app).
//
...
//     ready the puck replaces the default (Req 1.1, 1.4);
...
//
// **Validates: Requirements 1.1, 1.3, 1.5, 1.9**

import 'dart:typed_data';

import 'package:customer_app/core/maps/marker_icon_factory.dart';
import 'package:customer_app/core/theme/app_tokens.dart';
...
void main() {
  ...
  const position = LatLng(12.34, 56.78);
  ...
  group('buildRiderMarker', () {
    ...
    test('shows the custom puck once it is ready (Req 1.1, 1.4)', () {
      final marker = buildRiderMarker(
        position: position,
        bearing: 42,
        fallback: fallback,
    ...
    test('defaults the info window title to "Rider"', () {
      final marker = buildRiderMarker(
        position: position,
        bearing: 0,
        fallback: fallback,
      );
      expect(marker.infoWindow.title, 'Rider');
    });
  });
